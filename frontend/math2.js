...
let TIME_PER_QUESTION = 12;
let timeLeft = TIME_PER_QUESTION;
let timerInterval;

// ----------------- UTILITY -----------------
function getNumberBasedOnDifficulty() {
    return Math.floor(Math.random() * 16) + 5; // 5-25
}
...
function checkMathAnswer() {
    ...
    if(isNaN(userAnswer)){ alert("Enter a number!"); return; }
    clearInterval(timerInterval);

    if(userAnswer === mathAnswer){
        score += 10; if(score>30) score=30;
...
function handleNextQuestion(correct){
    mathQuestionNumber++;
    if(mathQuestionNumber>=MAX_QUESTIONS || attemptsLeft<=0) endMathGame();
    else setTimeout(loadMathQuestion,1000);
}
...
function endMathGame() {
    ...
    let starCount = 0;
    if(score>=30) starCount=3; else if(score>=20) starCount=2; else if(score>=10) starCount=1;
